...
void setup(void)
{
     ...
     * pin 4(P0.13), pin 5(P0.12), pin 6(P0.09), pin 7(P0.10), pin 9(P0.18),
     * pin 10(P0.19), pin 21(P0.14), and pin 24(P0.17).
    */
    gpio_output.pin_num = 18; // 18 means the P0.18 pin of the MCU, Pin 9 of the RAK4600 module. Fill in the MCU pin number here.
    gpio_output.dir = RUI_GPIO_PIN_DIR_OUTPUT;
    gpio_output.pull = RUI_GPIO_PIN_NOPULL;
    ret_code = rui_gpio_init(&gpio_output);
    if (ret_code != RUI_STATUS_OK)
        uart_log_printf("Output pin init error! %d\r\n", ret_code);

    gpio_input.pin_num = 19; // 19 means the P0.19 pin of the MCU, Pin 10 of the RAK4600 module. Fill in the MCU pin number here.
    gpio_input.dir = RUI_GPIO_PIN_DIR_INPUT;
    gpio_input.pull = RUI_GPIO_PIN_PULLUP;
    ret_code = rui_gpio_init(&gpio_input);
    if (ret_code != RUI_STATUS_OK)
        uart_log_printf("Input pin init error! %d\r\n", ret_code);
}
...
void loop(void)
{
    ...
    if (timeout_flag)
    {
        ...
        uart_log_printf("The state of pin %d is %d.\r\n", gpio_input.pin_num, pin_value);
