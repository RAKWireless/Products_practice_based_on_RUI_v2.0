...
int GPS_get_data(double* latitude,double* longitude,int16_t* altitude)
{
	...
	if(HasFix)	
	{
		...
		{			
			if(gps_data[GPS_SAMPLE_CNT/2].alt==0)
			{
				* latitude = gps_data[GPS_SAMPLE_CNT/2+1].lat;
				* longitude = gps_data[GPS_SAMPLE_CNT/2+1].lon;
				* altitude = gps_data[GPS_SAMPLE_CNT/2+1].alt;
			}else
			{
