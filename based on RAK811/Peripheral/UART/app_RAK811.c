...
void setup(void)
{
    ...
    ret_code = rui_uart_init(RUI_UART3, BAUDRATE_115200);
    if (ret_code != RUI_STATUS_OK)
        RUI_LOG_PRINTF("Uart init error! %d\r\n", ret_code);

    ret_code = rui_uart_mode_config(RUI_UART3, RUI_UART_USER);
    if (ret_code != RUI_STATUS_OK)
...
void loop(void)
{
    ...
    {
        ret_code = rui_uart_send(RUI_UART3, user_uart_buf, user_uart_len);
        if (ret_code != RUI_STATUS_OK)
