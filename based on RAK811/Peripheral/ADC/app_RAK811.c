...
uint32_t timer_interval = 1; // Timing time, in seconds.
bool timeout_flag = false;

RUI_GPIO_ST gpio_adc;
...
void setup(void)
{
     ...
     * RAK811-LF: pin 2, pin3, pin 4, pin 5, pin 15, pin 20, pin 22, pin 23.
     * RAK811-HF: pin 2, pin3, pin 4, pin 20, pin 22, pin 23.
    */
    gpio_adc.pin_num = 2; // Pin 2 of the RAK811 module.
    ret_code = rui_adc_init(&gpio_adc);
    if (ret_code != RUI_STATUS_OK)
        RUI_LOG_PRINTF("ADC init error! %d\r\n", ret_code);
}
...
void loop(void)
{
    ...
    {
        uint16_t adc_value;
        ret_code = rui_adc_get(&gpio_adc, &adc_value);
        if (ret_code != RUI_STATUS_OK)
            RUI_LOG_PRINTF("ADC conversion error! %d\r\n", ret_code);
        else
            RUI_LOG_PRINTF("The voltage at pin %d is %d mV.\r\n", gpio_adc.pin_num, adc_value);
