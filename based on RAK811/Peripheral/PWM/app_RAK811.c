...
void setup(void)
{
    ...
    /* The available pin of the RAK811 module is:
     * pin 2.
    */
    pwm_pin.rui_gpio.pin_num = 2; // Pin 2 of the RAK4200 module, PB12 pin of the MCU.
    pwm_pin.frequency = 10000;  // Set the frequency to 10KHz. Available range is 50Hz-500KHz
    pwm_pin.dutycycle = 25; // Duty cycle is 25%. Available range is 0%-100%.
    ret_code = rui_pwm_init(&pwm_pin);
    if (ret_code != RUI_STATUS_OK)
        RUI_LOG_PRINTF("PWM init error! %d\r\n", ret_code);
    else
    {
        ret_code = rui_pwm_start(&pwm_pin);
        if (ret_code != RUI_STATUS_OK)
            RUI_LOG_PRINTF("PWM start error! %d\r\n", ret_code);
    }
